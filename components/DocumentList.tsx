...
const getStatusStyle = (status: DocumentStatus, isImprocedente?: boolean, validationState?: 'PENDING_SELF' | 'PENDING_OTHERS' | 'COMPLETED' | 'ADMIN_CONCLUDED') => {
  ...
  if (validationState === 'COMPLETED' || status === 'OFICIALIZADO') {
...
const DocumentList: React.FC<DocumentListProps> = ({ documents, currentUser, onSelectDoc, onEditDoc, isReadOnly, isMyReferenceView }) => {
  ...
  const filteredDocs = useMemo(() => {
    return documents.filter(doc => {
      ...
      if (myViewMode === 'IMED' && !doc.conselheiros_providencia_nomes?.includes(currentUser.nome.toUpperCase())) return false;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-[#E5E7EB] shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="relative">
            ...
            <input type="text" placeholder="NOME OU PROTOCOLO..." className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-[11px] uppercase focus:border-blue-500" value={filters.term} onChange={(e) => setFilters({...filters, term: e.target.value})} />
          </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredDocs.map(doc => {
          ...
          const isAdminDespacho = ['ARQUIVADO', 'CONCLUIDO', 'EMAIL_RESPONDIDO', 'OFICIO_RESPONDIDO', 'NOTIFICAR'].includes(mainStatus);
          
          if (isAdminDespacho) {
            validationState = 'ADMIN_CONCLUDED';
            dynamicLabel = `✅ DESPACHO: ${STATUS_LABELS[mainStatus]}`;
          } else if (doc.status.includes('AGUARDANDO_VALIDACAO') && isInTrio) {
            if (!iValidated) {
              validationState = 'PENDING_SELF';
              dynamicLabel = "📋 ASSINATURA PENDENTE";
            } else if (confirmacoes.length < (doc.conselheiros_providencia_nomes?.length || 3)) {
              validationState = 'PENDING_OTHERS';
              dynamicLabel = "⏳ AGUARDANDO COLEGIADO";
            } else {
              validationState = 'COMPLETED';
              dynamicLabel = "✅ MEDIDA OFICIALIZADA";
            }
          }
